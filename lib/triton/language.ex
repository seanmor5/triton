defmodule Triton.Language do
  ...
  def __kernel_ast__(ast) do
    Macro.prewalk(ast, fn
      ...
      # `for i <- range(...), reduce: init` over a *dynamic* range becomes a
      # symbolic loop (scf.for natively). `static_range` comprehensions keep
      # plain Elixir semantics and unroll at trace time.
      {:for, meta,
       [{:<-, _gen_meta, [var, {:range, _range_meta, range_args}]}, [reduce: init],
        [do: [{:->, _clause_meta, [[acc_pattern], body]}]]]}
      when is_list(range_args) ->
        body_fun = {:fn, meta, [{:->, meta, [[var, acc_pattern], body]}]}
        {{:., meta, [__MODULE__, :loop_from_range]}, meta, [range_args, init, body_fun]}
  ...
  @doc false
  # Distinguishes `kernel(opts)` from `kernel(arg_specs)` in the functions
  # generated by `defkernel`. Argument specs may be keyword-shaped tuples such
  # as `{:ptr, {:f, 32}}`, so a list only counts as options when every key is
  # a known compile option.
  def __compile_opts_list__?(list) do
  ...
  @doc """
  A symbolic counted loop with a carried accumulator, compiled to a real
  `scf.for` on the native path (software-pipelined by Triton's passes) and
  interpreted as a plain reduction on the reference path.

  Unlike `static_range/1`, which unrolls at trace time and requires
  compile-time bounds, `loop/5` traces its body **once** and keeps the trip
  count symbolic, so bounds may be runtime scalar arguments:

      acc =
        loop(0, k, block_k, zeros(shape: {bm, bn}, dtype: float32()), fn kk, acc ->
          ...
          acc + dot(a, b)
        end)

  Inside `defkernel`, `for kk <- range(0, k, block_k), reduce: acc do ... end`
  is rewritten to this construct automatically.

  The body function receives the scalar `{:s, 32}` induction variable and the
  carried value, and must return the next carried value with the same shape
  and type. Side-effecting ops (`store`, atomics) inside the loop body run on
  the native path but are not yet threaded by the reference interpreter.
  """
  def loop(start, stop, step, init, body_fun) when is_function(body_fun, 2) do
    unique = System.unique_integer([:positive])

    index =
      :"__triton_loop_index_#{unique}__"
      |> Expr.parameter()
      |> Map.merge(%{shape: {}, type: {:s, 32}})

    case init do
      init when is_tuple(init) and Kernel.>(tuple_size(init), 1) ->
        # Multiple carried values: one loop parameter per element; the body
        # function receives them as a tuple it can pattern match, and the
        # loop expression itself returns a tuple of element projections.
        arity = tuple_size(init)
        last = Kernel.-(arity, 1)

        carries =
          for position <- 0..last do
            Expr.parameter(:"__triton_loop_carry_#{unique}_#{position}__")
          end

        body = body_fun.(index, List.to_tuple(carries)) |> Expr.wrap()

        unless match?(%Expr{op: :tuple}, body) and length(body.args) == arity do
          raise ArgumentError,
                "loop body must return a #{arity}-tuple matching its carried values"
        end

        inits = init |> Tuple.to_list() |> Enum.map(&Expr.wrap/1)

        loop_expr =
          Expr.new(
            :for_loop,
            [Expr.wrap(start), Expr.wrap(stop), Expr.wrap(step) | inits],
            index: index,
            carries: carries,
            body: body
          )
        ...
        0..last
        |> Enum.map(&Expr.new(:tuple_element, [loop_expr], index: &1))
        |> List.to_tuple()

      init ->
        carry = Expr.parameter(:"__triton_loop_carry_#{unique}__")
        body = body_fun.(index, carry) |> Expr.wrap()

        Expr.new(
          :for_loop,
          [Expr.wrap(start), Expr.wrap(stop), Expr.wrap(step), Expr.wrap(init)],
          index: index,
          carries: [carry],
          body: body
        )
  ...
  @doc false
  # Target of the `for VAR <- range(...), reduce: INIT` rewrite performed by
  # `__kernel_ast__/1`. Accepts the same argument shapes as `range/1..4` but
  # allows runtime (traced) bounds.
  def loop_from_range(range_args, init, body_fun) do
    {start, stop, step} = loop_range_bounds!(range_args)
    loop(start, stop, step, init, body_fun)
  end

  defp loop_range_bounds!([opts])
       when is_list(opts) do
    if Keyword.keyword?(opts) do
      opts =
        Keyword.validate!(opts, [
      ...
      {Keyword.get(opts, :start, 0), required_keyword!(opts, :stop, :range),
       Keyword.get(opts, :step) || 1}
    else
      raise ArgumentError, "range options must be a keyword list, got #{inspect(opts)}"
    end
  end

  defp loop_range_bounds!([stop]), do: {0, stop, 1}
  defp loop_range_bounds!([start, stop]), do: {start, stop, 1}

  defp loop_range_bounds!([start, stop, step_or_opts]) do
    cond do
      is_list(step_or_opts) -> {start, stop, 1}
      is_nil(step_or_opts) -> {start, stop, 1}
      true -> {start, stop, step_or_opts}
    end
  end

  defp loop_range_bounds!([start, stop, step, _opts]), do: {start, stop, step || 1}
  ...
  def +value, do: if(Kernel.is_number(value), do: value, else: Expr.wrap(value))

  # Negating a plain number folds at trace time, so numeric literals like
  # `-1.0e30` stay plain numbers for options such as `full(value: ...)`.
  def -value when Kernel.is_number(value), do: Kernel.-(value)
